    ...
    class peliculas extends conexion{
        ...
        public $response = [
         ...
         public function insertarPelicula($json){
            ...
            if (!isset($datos['titulo']) ||  !isset($datos['genero']) || !isset($datos['duracion']) || !isset($datos['sinopsis'])|| !isset($datos['imagen'])) {
               ...
               // faltan validaciones para la fecha de lanzamiento 
                $query = "INSERT INTO peliculas (id_pelicula, titulo, genero, duracion, sinopsis, imagen) VALUES (NULL, '$titulo','$genero', '$duracion','$sinopsis', '$imagen')";            
        ...
        public function actualizarPelicula($json){
            ...
            #si no estan los datos requeridos
            if (!isset($datos['id_pelicula']) || !isset($datos['titulo']) || !isset($datos['genero']) || !isset($datos['duracion'])|| !isset($datos['sinopsis']) || !isset($datos['imagen'])) {
                $this->response['status'] = "error";
                $this->response['result'] = array(
                    "error_id" => "400",
                    "error_msg" => "Datos enviados incompletos o con formato incorrecto"
                ...
                // codigo para levantar la imagen guardarla en el server y guardar la ruta en la base de datos
                // faltan validaciones para la fecha de lanzamiento 
                $query = "UPDATE peliculas SET titulo = '$titulo', genero = '$genero', duracion = '$duracion', sinopsis = $sinopsis, imagen = '$imagen' WHERE id_pelicula = '$id'";            
